...
`@kazamaswap/smart-router` is a SDK for getting best trade routes from Pancakeswap AMM.

## Install

```bash
$ pnpm add @kazamaswap/smart-router
```
...
```bash
$ pnpm add viem graphql-request @kazamaswap/sdk @kazamaswap/tokens
```
...
```typescript
import { Native } from '@kazamaswap/sdk'
import { SmartRouter } from '@kazamaswap/smart-router/evm'
...
```typescript
import { CurrencyAmount, TradeType } from '@kazamaswap/sdk'
...
import { SmartRouter, SmartRouterTrade, SMART_ROUTER_ADDRESSES, SwapRouter } from '@kazamaswap/smart-router/evm'
